...
class Concurrent:
    ...
    def __init__(self, concurrency=10, state_file='', save_mid_result=True, check_done=True):
        ...
        # self.donetasks = []
        # if self.check_done:
        #     self.donetasks = self.fh.read_done_tasks()
    ...
    def load_cache(self):
        LOGGER.info(u'加载缓存数据...')
        for c in self.fh.read_cache():
            self.caches[c['taskid']] = c

        if self.caches:
            LOGGER.info(u'缓存数据加载完成，共加载%d条数据.' % len(self.caches))
        else:
            LOGGER.info(u'缓存数据加载完成，共加载0条数据.')
    ...
    #     """
    #     taskid = task['taskid']
    #     if self.donetasks and taskid in self.donetasks:
    #         return True
    #     else:
    #         return False
    ...
    def add_jobs(self, tasks):
        ...
        if self.caches:
            for task in tasks:
                if task['taskid'] in self.caches.keys():
                    self.results.put(self.caches[task['taskid']])
        ...
        LOGGER.info(u'添加完成，共添加{}条任务.'.format(self.todo))
        return self.todo
    ...
    def start(self, tasks):
        ...
        LOGGER.info(u'所有任务已结束，用时 {}s.任务成功{}个，失败{}个.'.format(int(timer.total), self.success, self.failed))
